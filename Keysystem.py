...
async def createkey(ctx, duration: str):
    ...
    db.insert({'key': key, 'duration': duration, 'used': False})
...
async def redeem(ctx, key: str):
    ...
    if record:
        ...
        if not record['used']:
            ...
            await ctx.send(embed=embed)
            db.update({'used': True}, tinydb.Query().key == key)
            asyncio.create_task(remove_role_after_duration(ctx.author, role, record['duration']))
        else:
